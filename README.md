...
- No painel, configure a **duração da medição** (padrão 4 s) e o **limite de trim ±dB** (padrão ±36, ajustável até ±60) e clique em **"Regular ganhos de todos os canais"**. A medição é **armada por canal**: cada canal fica "aguardando sinal" e a janela só começa a contar quando o sinal chega nele — fontes que tocam em momentos isolados (tons, surdo) são capturadas quando tocarem. A medição arma no limiar de **Sensibilidade do canal** (Avançado) — canal com muito vazamento: suba a Sensibilidade dele acima do vazamento para que só o toque direto dispare. Canais com perfil **Bateria** fecham a medição por **contagem de hits (4)** em vez de tempo — um transiente isolado nunca define o trim sozinho. Ao fim da janela de cada canal, ele recebe `trim = target − pico medido` (limitado ao ±limite). Canal sem sinal em 90 s fica intocado. Também há regulagem individual: botão "Regular ganho" no plugin do canal e botão "Reg" em cada linha do painel/modo compacto.
